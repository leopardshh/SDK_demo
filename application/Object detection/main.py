from Objectdetect import Ui_Objectdetect
import sys, os
sys.path.insert(0, '..')
sys.path.insert(0, '../../')
...
class PrintDialog():
    ...
    def __init__(self):
        self.dialog = QDialog()
        self.dialog.setWindowTitle('Detection Result')
...
class Ui_Objectdetect(QMainWindow,Ui_Objectdetect):
    printSignal = QtCore.pyqtSignal(list)
    def __init__(self,parent=None):
        super(Ui_Objectdetect,self).__init__(parent)
        self.setupUi(self)
        self.Callback()
    ...
    def update_frame(self):
        # Original video
        ret, stereo_img = self.camera.grab_rgb(gs.FRAME_GRAB.LEFT)
        self.obj_res =  self.gs_objdetect.get_object_detection_result(stereo_img)
        self.printSignal.emit(self.obj_res)
        disp_objdetect = self.gs_objdetect.grab_object_detection_image()
        if ret:
            # transfer color zone into RGB
            int8_image = cv2.convertScaleAbs(disp_objdetect)
    ...
    def saveImage(self):
        ...
        if file_name:
            ret, stereo_img = self.camera.grab_rgb(gs.FRAME_GRAB.LEFT)
            obj_res =  self.gs_objdetect.get_object_detection_result(stereo_img)
            disp_objdetect = self.gs_objdetect.grab_object_detection_image()
            cv2.imwrite(file_name, disp_objdetect)
